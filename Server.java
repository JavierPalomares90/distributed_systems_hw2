...
public class Server
{
    ...
    private static abstract class ClientWorkerThread implements  Runnable
    {
        ...
        // Send the updated seats list to every peer
        private String updateRequestor(int serverID)
        {
            String seatsJson = getSeatsAsJson(seats);
            String msg = UPDATE + " " + seatsJson;
            try
            {
                msgRequestor(msg, serverID);
            }catch(InterruptedException e)
            {
                e.printStackTrace();
                return "Unable to update peer's seat list";
        ...
        private void msgRequestor(String msg, int serverID) throws InterruptedException
        {
            ExecutorService executor = Executors.newFixedThreadPool(5);
            List<Callable<Integer>> workers = new ArrayList<>();

            Callable<Integer> worker = new MessagePeerThead(peers.get(serverID), msg);
            workers.add(worker);
